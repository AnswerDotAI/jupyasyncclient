...
async def kc(jp_server):
    async with httpx.AsyncClient() as http:
        ...
        JmsgQueues(client, queues=("shell", "jmsg"), merge=dict(iopub="jmsg", stdin="jmsg"))
...
async def _wait_for_iopub_status(kc, parent_msg_id: str, state: str, timeout: float = TIMEOUT):
    ...
    try: return await kc.jmsgq.jmsg_for("status", pred=_pred, timeout=timeout)
...
async def _wait_for_shell_reply(kc, parent_msg_id: str, msg_type: str, timeout: float = TIMEOUT):
    ...
    while True:
        ...
        try: msg = await kc.jmsgq.get("shell", timeout=t)
...
class TestSubshells:
    ...
    async def test_subshell_concurrency_execute_vs_complete(self, kc):
        ...
        exec_id = kc.execute("import time; time.sleep(1.0); 123", subshell_id=sid)
        ...
        comp = await kc.complete_request(code="impor", cursor_pos=5, timeout=0.6)
