...
async def kc(jp_server):
    async with aiohttp.ClientSession() as http:
        async with http.post(jp_server["base_url"] + "/api/kernels", json={"name": "python3"}) as r:
            r.raise_for_status()
            model = await r.json()
        kernel_id = model["id"]
        ...
        try:
            ...
            await client.aclose()
            try:
                await http.delete(jp_server["base_url"] + f"/api/kernels/{kernel_id}")
            except Exception:
                pass
...
class TestJupyAsyncKernelClient:
    async def test_execute_interactive_and_output_hook(self, kc):
        ...
        def hook(msg):
            if msg["header"]["msg_type"] == "stream" and "hello" in msg["content"].get("text", ""):
                got.set()
